  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newDocument: Document = {
        ...
        name: uploadType === 'other' ? uploadFile.name : `${uploadType.charAt(0).toUpperCase() + uploadType.slice(1)} Document`,
        status: 'pending',
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {requiredDocuments.map((doc, idx) => {
              const uploaded = documents.find(d => d.type === doc.type && d.status !== 'expired' && d.status !== 'rejected')
                ...
                <div key={idx} className="flex items-center space-x-3 p-3 border rounded-md">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {uploaded ? `Status: ${uploaded.status.charAt(0).toUpperCase() + uploaded.status.slice(1)}` : (doc.required ? 'Status: Missing (Required)' : 'Status: Optional')}
                    </p>
                  </div>
                  {!uploaded && (
                    <Button
              ...
              <div className="grid gap-2">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="authority">Operating Authority</SelectItem>
                    <SelectItem value="w9">W-9 Form</SelectItem>
                    <SelectItem value="contract">Broker-Carrier Agreement</SelectItem>
...
// Helper function for document status icons
