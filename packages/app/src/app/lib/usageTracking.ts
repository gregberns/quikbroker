...
export async function trackApiUsage(data: UsageTrackingData): Promise<void> {
  try {
    ...
    console.log(`API Usage: ${data.endpoint} from ${data.ip} - Status: ${data.response_status}, Time: ${data.response_time}ms`);
...
export function withUsageTracking<T extends any[]>(
  ...
  return async (...args: T): Promise<Response> => {
    ...
    const dotNumber = dotNumberIndex > 0 && dotNumberIndex < pathParts.length 
    ...
    try {
      ...
      await trackApiUsage({
        ...
        dot_number: dotNumber || url.searchParams.get('dot_number') || undefined,
